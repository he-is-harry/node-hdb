...
function Connection(settings) {
  ...
  this._autoReconnect = false; // Reconnect if a packet is to be sent
}
...
Connection.prototype.send = function send(message, options, receive) {
  if (this._autoReconnect) {
    ...
    return this.reconnect(function (err) {
      if (err) {
        receive(new Error("Failed to reconnect"));
      } else {
        self.send(message, options, receive);
      }
    });
  }
  ...
  function onTimeout() {
    self._state.receive = undefined;
    var receiveTimeoutError = 'Socket receive timeout (receive took longer than ' + options.communicationTimeout + ' ms)';
    var err = new Error('Connection closed: ' + receiveTimeoutError);
    err.code = 'EHDBCLOSE';
    self.reset(err, function () {
      self._autoReconnect = true;
      receive(new Error(receiveTimeoutError));
    });
  }
...
Connection.prototype.enqueue = function enqueue(task, options, cb) {
  ...
  if (((!this._socket || this.readyState === 'closed') && !this._autoReconnect) || !this._queue) {
    var err = new Error('Connection closed');
    err.code = 'EHDBCLOSE';
    if (cb) {
      return cb(err);
...
// Reset will destroy the socket, clear the queue, reset the state of the Connection
// to a new connection, except that the options are persisted. Intended to be used
// when needing a reconnect, since the connection will immediately run tasks
Connection.prototype.reset = function (err, cb) {
  var self = this;

  // Wait for the socket to fully close before updating the other fields, since
  // the socket listeners may alter the connection
  function onClose() {
    self._socket = undefined;
    // If socket close event is listened to, the queue will be cleared already
    // As such, it is possible the clear queue line might never be called
    if (self._queue) {
      self._clearQueue(err);
    }
    // Create a queue that will immediately run the next task
    self._queue = new util.Queue(true);

    self._state = new ConnectionState();
    self._statementContext = undefined;
    self._transaction = new Transaction();
    self._transaction.once('error', function onerror(err) {
      self.destroy(err);
    });
    self._initialHost = undefined;
    self._initialPort = undefined;
    self._redirectHost = undefined;
    self._redirectPort = undefined;
    self.host = self._settings['host'];
    self.port = self._settings['port'];
    self._redirectType = common.RedirectType.REDIRECTION_NONE;
...
// Reconnect does not support redirection currently, changes could be replicated
// from Client to support this functionality
Connection.prototype.reconnect = function (cb) {
  var self = this;
  this._autoReconnect = false;
  // Connect options should be maintained in the Connection object
  var options = util.extend({host: this.host, port: this.port}, this._settings);
  this.open(options, function (err) {
    if (err) return cb(err);
