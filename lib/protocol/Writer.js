...
Writer.prototype[TypeCode.REAL_VECTOR] = function writeRealVector(value, length) {
  if (Array.isArray(value)) {
    // Validate length
    if (value.length === 0) {
      throw createInvalidLengthError('REAL_VECTOR');
    } else if (length !== 0 && value.length !== length) {
      throw createMismatchTargetLengthError('REAL_VECTOR');
    }
    this.push(createVectorOutBuffer(value, 4, function (value, buffer, offset) {
    ...
    if (value.length === 0 || value.length % 4 !== 0) {
      ...
      if (fvecsLength === 0 || fvecsLength !== value.length / 4 - 1) {
        throw createInvalidLengthError('REAL_VECTOR');
